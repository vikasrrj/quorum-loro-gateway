...
fn snapshot_accepts_update_from_client_offline_before_checkpoint() {
    ...
    initial.get_text("text").insert(0, "base").unwrap();
    initial.commit();

    let initial_updates = initial.export(ExportMode::all_updates()).unwrap();
    ...
    server.get_text("text").insert(4, "-server").unwrap();
    server.commit();

    // This represents the current full-replay recovery path.
    let complete_history = server.export(ExportMode::all_updates()).unwrap();

    // This is the proposed checkpoint base.
    let snapshot = server.export(ExportMode::Snapshot).unwrap();
    ...
    let checkpoint_replay = LoroDoc::from_snapshot(&snapshot).unwrap();
...
fn snapshot_alone_does_not_preserve_causally_pending_import() {
    ...
    remote_client.get_text("text").insert(4, "-one").unwrap();
    ...
    remote_client.get_text("text").insert(8, "-two").unwrap();
    remote_client.commit();
    ...
    let pending_status = full_replay.import(&second_update).unwrap();
    assert!(
    ...
    let pending_status = checkpoint_source.import(&second_update).unwrap();
    assert!(
        pending_status.pending.is_some(),
        "the checkpoint source was expected to contain a pending import"
    );

    let snapshot = checkpoint_source.export(ExportMode::Snapshot).unwrap();
